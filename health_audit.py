...
def _load_schema_contract():
    """Load the SCHEMA.json sheets-contract (TASK-167). Returns {} if absent.

    SCHEMA.json is a derived artifact generated by
    scripts/generate_sheets_schema.py from the code constants — never edited by
    hand. Absence/parse-error degrades to {} so the check skips rather than
    crashing (advisory contract).
    """
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "SCHEMA.json")
    try:
        with open(path, encoding="utf-8") as fh:
            return json.load(fh).get("sheets", {})
    except Exception:
        return {}


def _required_columns_result(header_by_sheet, contract):
    """Pure drift detector (no gc) — TASK-167 Layer-2 logic, Layer-1 tested.

    For each sheet present in `header_by_sheet`, compare its live header to the
    contract spec. Returns a list of human-readable issue strings (empty = no
    drift):
      - mode "exact": live column set must equal contract `columns`; reports
        `missing` and `unexpected` columns.
      - mode "required_subset": contract `required` must be a subset of live;
        `forward_optional` columns are NOT required (post_analysis D6-D25 grow
        forward-only — TASK-177 — so their absence is never drift).
    """
    issues = []
    for name, spec in contract.items():
        if name not in header_by_sheet:
            continue
        live = header_by_sheet[name]
        live_set = set(live)
        mode = spec.get("mode")
        if mode == "exact":
            cols = spec.get("columns", [])
            missing = [c for c in cols if c not in live_set]
            extra = [c for c in live if c not in set(cols)]
            if missing:
                issues.append(f"{name}: missing {missing}")
            if extra:
                issues.append(f"{name}: unexpected {extra}")
        elif mode == "required_subset":
            missing = [c for c in spec.get("required", []) if c not in live_set]
            if missing:
                issues.append(f"{name}: missing required {missing}")
    return issues


def check_08_required_columns(gc):
    """Q2: Live sheet headers match the SCHEMA.json contract (TASK-167).

    Layer-2 drift check — reads live headers (row 1) and delegates to the pure
    _required_columns_result. **Advisory WARNING only, never CRITICAL/exit-1.**
    With no gc (CI / local), returns INFO skip; the detector logic itself is
    covered by Layer-1 tests (test_sheets_schema_contract_v1.py), so this path
    is verified without any live Sheets read.
    """
    if gc is None:
        return CheckResult("Q2", "Schema contract (columns)", "Data quality",
                           INFO, "Skipped (no Sheets access)")

    contract = _load_schema_contract()
    if not contract:
        return CheckResult("Q2", "Schema contract (columns)", "Data quality",
                           INFO, "Skipped (SCHEMA.json absent)")
    ...
    for name in contract:
        if name not in sheets:
            continue
        try:
            header_by_sheet[name] = _ha_row_values(gc, sheets[name], 1)
        except Exception:
            header_by_sheet[name] = []  # surfaces as drift below, not a crash

    issues = _required_columns_result(header_by_sheet, contract)
    if not issues:
        return CheckResult("Q2", "Schema contract (columns)", "Data quality",
                           PASSED,
                           f"All contracted columns present ({len(header_by_sheet)} sheets)")

    return CheckResult("Q2", "Schema contract (columns)", "Data quality",
                       WARNING, f"{len(issues)} sheet(s) drift vs contract",
                       "; ".join(issues))
