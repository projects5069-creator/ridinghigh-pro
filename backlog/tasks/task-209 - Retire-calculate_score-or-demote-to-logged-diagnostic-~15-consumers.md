...
--- הפניה נכנסת 2026-08-08 (מ-TASK-254 שנסגר) ---
`score_analytics` (הטאב + `score_analytics.py`) מצטרף להיקף-ההכרעה של אשכול
ה-Score. TASK-254 חקר ומצא: `orchestrator_eod.py:208/:226` בונים
`ScoreAnalytics()` ללא הזרקות ⇒ הטאב n=0 **מבנית**, בלי קשר לדאטה.
**ההמלצה שם: לקפל את הטאב לרשימת-הפרישה כאן, ולא לחווט reader+writer.**
נרשם כאן כי לפני 8/8 אף אחד מ-208/209 לא הזכיר את הטאב, וסגירת 254 הייתה
מאבדת את המסקנה.

--- הערה 2026-08-08 (מבניית תוכנית-העבודה) — פתוח להכרעה, לא הוכרע ---
**T-202 (שומר-הריק של Q1/Q4 ב-health_audit) יושב היום בתוך ההיקף של התיק הזה**
— הוא נכנס לכאן כפריט (4) של ספיגת-ה-E2E-audit: `check_07_score_range` (:738)
ו-`check_12_score_weights_sum` (:1008), "לרענן/להחריג בהחלטת-הסיום".

**המשמעות שלא נאמרה עד היום:** כל עוד T-202 כלוא כאן, **שער 2·c5 תקוע עד
אוקטובר** — כי התיק הזה קפוא על ורדיקט HYP-002 (רולינג 5/8).

**וההבחנה שמצדיקה לשקול פיצול:** T-202 הוא **קוד-בריאות, לא קוד-מסחר**.
הוא לא נוגע ב-Score עצמו, לא בשער-כניסה ולא בשום נתיב-כתיבה — הוא רק מונע
מ-Q1 להחזיר PASSED כשאפס ערכים נפענחו (וזה המצב היום: Score 0% מילוי מיולי,
כלומר הבדיקה ירוקה-על-ריק כבר חודש). שינוי כזה מותר בתוך חלון-המדידה.

**השאלה הפתוחה:**
1. לפצל את T-202 לתיק נפרד ולבצע בחלון?
   א. **כן** — c5 מוריק כבר באוגוסט; ‏health_audit יתחיל להחזיר WARNING קבוע
      על Score-ריק (רעש מכוון, מצביע על אמת) עד שגורל-Score יוכרע.
   ב. לא — c5 נשאר תקוע עד אוקטובר; שער 2 יישאר לכל היותר 4/5.
   ג. לפצל אבל לתזמן אחרי 4/9 — פשרה: לא נוגעים בחלון, אבל גם לא ממתינים
      לוורדיקט.
   ⚠️ אין ברירת-מחדל מוצעת. **לא הוכרע** — ייענה כשנגיע לכאן.
