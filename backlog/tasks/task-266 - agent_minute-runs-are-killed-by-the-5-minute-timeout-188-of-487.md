...
════════════════════════════════════════════════════════════════════════════════
## מדידת-שני 2026-08-10 — הנתונים האמיתיים, לפני הרצת השער (append 14:5x)

נמדד job-level על 365 ריצות (13:25–19:23Z), במקביל לחקירת הכפילויות:

```
בוקר   13:25-14:40Z  n=76  חציון=58ש  p90=83ש   max=112ש  חפיפה=53%  שיא-מקבילי=3
צהריים 15:00-16:00Z  n=61  חציון=82ש  p90=102ש  max=156ש  חפיפה=87%  שיא-מקבילי=4
אחה"צ  17:00-18:00Z  n=62  חציון=101ש p90=152ש  max=316ש  חפיפה=95%  שיא-מקבילי=5
מבוטלות ביום: 1
```

⚠️ **שתי הנחות של ההכרעה הופרכו על נתוני-אמת:**
1. "8/7 מייצג את המשטר החדש" — לא: 8/7 היה 48ש/10.2% בלי סיגנלים; היום עם
   סיגנלים אמיתיים החציון 58→101ש והחפיפה 53→95% לאורך היום.
2. "חציון<60ש ⇒ החפיפה נעלמת" — לא: הכפילות של היום נגרמה מריצה שהמתינה
   **בתור** 311ש (מחסור-runners), לא ממשך-ריצה. עיכוב-תור מייצר התחלה-בו-זמנית
   גם כשהריצות קצרות. המדד הנכון = התחלות-צמודות (<15ש), לא "חפיפה%".

מחיר-concurrency נמדד בסימולציה על היום: **137/365 נזרקות (37.5%, חסם עליון)**,
כיסוי-דקות 359→228; אף ריצת-ENTER לא נזרקת; הכפילות נמנעת ע"י סידור.
הכרעת עמיחי 10/8: concurrency ממתין 3-4 ימים לנתוני גלאי-הכפילויות.

## ⛔ שער-הקבלה רץ 2026-08-10 אחרי הסגירה — **FAIL**

```
total=431  cancelled=44  => 10.2%   (סף: <5.0%)
last-60: median=314s  p90=316s  max=317s   (=חיתוך ה-timeout, הד-300ש)
overlapping pairs: 57/58 = 98.3%
real Sheets 429 in 5 sampled runs: 145 שורות (15..36 לריצה)
VERDICT: FAIL — התיק נפתח-מחדש עם המספרים האלה, כלשון השער.
```

⚠️ מתקן את ה-append הקודם מהיום: "מבוטלות ביום: 1" נמדד ב-14:10 —
הסערה התפתחה אחרי-כן. סוף-יום: **44**. התסריט שהקלף-הנגד של ההכרעה
צפה ("אם סערת-429 תחזור... כל יום כזה מאבד כתיבות") התממש **ביום 1**.
ההכרעה ג׳ הימרה ש-8/7 מייצג — ההימור הופרך ביום המדידה הראשון.
ההכרעה בין א׳ (העלאת תקרה) ל-ב׳ (תיקון האיטיות) חוזרת לשולחן — הפעם
על נתוני-אמת. נימוק #2 של ההכרעה המקורית (התקרה כמפסק-זרם שמגביל
זוחלות מקבילות) רלוונטי מתמיד: החפיפה 98.3% בשעת-הסערה.

## הכרעת עמיחי 2026-08-10 — אפשרות ב': לתקן את השורש, לא להעלות תקרה
timeout-minutes: 5 נשאר. מטפלים במה שמאט את הריצה, לא בתקרה.
הנימוק, מגוף התיק עצמו: בשתי ריצות שנקראו לוג-מלא, ה-orchestrator עלה תוך ~52ש ואז
נתקע בשרשרת 429 עד שנהרג ב-~317ש — כלומר הריצות **זחלו, לא עבדו**. גם ה-flushes שכן
הספיקו לרוץ נכשלו על 429. תקרה גבוהה יותר לא הייתה מצילה אותן, ולפי נימוק #2 של הכרעת
8/8 היא אף מעמיקה את הסערה (מכפילה את מספר הזוחלות המקבילות על אותה מכסה ממוצה).
⚠️ שער-הקבלה של התיק רץ 10/8 אחרי הסגירה והחזיר **FAIL**: 44/431 מבוטלות = 10.2%
מול סף 5.0%; last-60 median=314s (הד-חיתוך); חפיפה 98.3%; 145 שורות-429 בחמש ריצות.
⇒ ההימור של הכרעת 8/8 (ש-8/7 מייצג את המשטר החדש) הופרך ביום המדידה הראשון.
⚠️ הסטטוס לא שונה. הביצוע תלוי-חלון: .github/workflows/ הוא נתיב מוגן ושינוי משטר-ריצות.
