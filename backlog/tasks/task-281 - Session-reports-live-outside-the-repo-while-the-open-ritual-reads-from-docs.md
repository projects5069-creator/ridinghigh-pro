...
## הכרעת עמיחי 2026-08-10 — אפשרות א': דוחות-סשן בריפו בלבד
ה-handoff נכתב ישירות ל-docs/SESSION_HANDOFF_<תאריך>.md. התיקייה שמחוץ לריפו נשארת
לחומרי-גלם בלבד (recon, tsv, לוגים).
הנימוק: אפשרות ב' דורשת שער נוסף עם בקרה דו-כיוונית — כלומר פותרת בעיה ע"י יצירת עוד
מופע של אותה מחלקת-כשל. באותו יום אחד (8/8) נרשמו שלושה מקרים שבהם שער "עבר" בלי
שנבדק בכיוון השני, ובנוסף ההעתקה הידנית נשכחה. אפשרות א' מסלקת את הצורך בשער לגמרי.
⚠️ שני קבצי docs/auto-dancer/queue/* — **לא הוכרעו**. אין למחוק על סמך "לא מוכר".
