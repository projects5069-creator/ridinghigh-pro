...
## הכרעת עמיחי 2026-08-10 — לסמן, לא למחוק
198 השורות מחוץ לחלון-הריצה מסומנות ואינן נמחקות. הנימוק: רק 4 מתוך 198 יוחסו בוודאות
(2%), ומחיקה על סמך 2% ודאות היא הימור; סימון הפיך אינו.
⚠️ ממצא חדש 10/8 שמחזק את ההכרעה: הגזירה-מחדש מהעותק המקומי מראה שערכי ScannedUniverse
4, 5 ו-7 מופיעים **בשתי** הקבוצות (בתוך-החלון ומחוצה לו), ו-3 מופיע גם בנקיות.
כלומר חתימת-הזיהום שהתיק מגדיר ("שורות 1 ו-3 הן התת-קבוצה הוודאית") **אינה מפרידה** —
שני הסיווגים אינם מסכימים, ולכן אין קריטריון-מחיקה בטוח בכלל.

## ממצא נוסף 2026-08-10 — 16 שורות ביום אחד, **בתוך** חלון-הריצה הלגיטימי
מהגזירה-מחדש של TASK-261 (עותק מקומי, אפס קריאות Sheets): מתוך 21 השורות שנפלו
בחלון 16:00-17:59, **16 מהן הן מ-3/8 בלבד**. הקולקטור אמור לכתוב 1-2 שורות ליום
(ריצת-EOD + מעבר-ריפוי). ובנוסף: ScannedUniverse משתנה בתוך אותה שעה (4/5/7) בעוד
WithBorrowData נעוץ ב-4 בכל 16 השורות.
⇒ זה **לא בדיקות** (הן מחוץ לחלון) ו**לא ריצה תקינה**. גורם שלישי שלא זוהה.
נרשם כאן ולא כתיק חדש: בודק-הכפילות החזיר **62% חפיפה עם התיק הזה** — אותו טאב,
אותה חקירה, אותו שדה.
