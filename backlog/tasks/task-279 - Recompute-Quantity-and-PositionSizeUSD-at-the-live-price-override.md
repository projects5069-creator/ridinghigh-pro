...
--- עדכון 2026-08-08: התלות של 276 הפכה רשמית, וזה הנימוק המכריע להכרעה שם ---

‏**TASK-276 (תקרת-חום) הוכרעה 8/8 לאופציה ג׳ — לא נכנסת לפני החלון, אלא אחרי
4/9 ויחד עם התיק הזה.** שורת-ההצלבה שלמעלה ("inherits correctness") כבר הייתה
כתובה; מה שהשתנה הוא שהיא הפכה מהערה **לנימוק המכריע** בהכרעת-תזמון.

**הנימוק, במספר אחד:** ‏`gate3_risk.py` מדד 8/8 **205 מתוך 291 שורות מפרות**
את אינווריאנטי-הזהות — אותו מספר שכתוב בגוף התיק הזה. תקרת-החום סוכמת
Σ‏0.10·E×Q על שורות OPEN. ⇒ לו הייתה נכנסת לפני התיק הזה, **היא הייתה חוסמת
כניסות אמיתיות על בסיס חישוב שידוע שחלקו שגוי** — הצרכן היה נוחת לפני הספק.

⇒ **סדר-ביצוע מחייב: 279 לפני 276.** אין לפרוס את תקרת-החום לפני שהכמויות
תוקנו ו-`gate3` מחזיר אפס-מפרות בהיקף.

⚠️ **ולידיעה בעת ביצוע התיק הזה:** ה-recon של 276 (8/8) זיהה ש-`execute()`
שולח את ההזמנה ב-`order_manager.py:121` וכותב את השורה רק ב-`:166`. כל עבודה
עתידית שנוגעת בסמנטיקת-הכתיבה — כולל זו — צריכה להיות מודעת לפער הזה בין
שליחת-ההזמנה לרישום. ראיות: `~/rhpro_audit_run/TASK276_RECON.md`.
