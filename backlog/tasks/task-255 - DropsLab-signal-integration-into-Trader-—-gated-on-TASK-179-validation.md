...
## הכרעת עמיחי 2026-08-10 — לסגור כלא-רלוונטי
ההוראה לחווט סיגנלי DropsLab ל-Trader בוטלה ב-10/8 (הפרדה מוחלטת בין שלוש המערכות,
אפס ייבוא בזמן-ריצה בין הריפואים; PK v4.21). שלוש חלופות נתיב-הדאטה שבגוף התיק מפרות
את ההכרעה הזו, ובנוסף נמדד 0 קוד-קורא קיים (שלושת האזכורים היחידים הם שורות-הערה).
⚠️ **הסטטוס לא שונה כאן** — הרישום בלבד. הסגירה בפועל היא פעולה נפרדת של עמיחי.
הערה מבדיקת 10/8: DropsLab עצמה **חיה** (42 ריצות ב-30 יום, אחרונה 10/8 22:06Z) —
הסגירה היא בגלל ההכרעה, לא בגלל שהמערכת מתה.

## נסגר 2026-08-10 — Done = **לא רלוונטי**, לא "בוצע"
⚠️ הבהרת-סמנטיקה: `backlog/config.yml` מכיר שלושה סטטוסים בלבד ואין בהם "Cancelled".
הסטטוס Done כאן פירושו **התיק אינו רלוונטי עוד**, ולא שהעבודה בוצעה. שום קוד לא נכתב.
הנימוק במלואו בסעיף שמעליו: ההוראה לחווט סיגנלי DropsLab ל-Trader בוטלה ב-10/8
(הפרדה מוחלטת בין שלוש המערכות, אפס ייבוא בזמן-ריצה; PK v4.21), ושלוש חלופות
נתיב-הדאטה שבגוף מפרות אותה.
