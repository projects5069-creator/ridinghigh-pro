...
def resolve_whipsaw(entry_price, minute_bars_df,
    ...
    A daily WHIPSAW means a single DAILY bar showed BOTH the TP (price drops
    tp_frac) and the SL (price rises sl_frac) — so we KNOW both levels were reached
    that day; daily granularity just can't order them. The minute bars resolve it
    ONLY if they capture BOTH touches in SEPARATE bars — the earlier one decides:

      'WIN'        — first minute touching TP (low <= TP) precedes first touching SL
      'LOSS'       — first minute touching SL (high >= SL) precedes first touching TP
      'UNRESOLVED' — only ONE side appears in the minutes (the other extreme never
                     printed on the IEX feed), OR both first appear in the SAME minute
                     (sub-minute, unresolvable), OR no/invalid input.

    Critically: a one-sided minute view is NOT a verdict. Since daily proves both
    levels were hit, seeing only one side means the other is hidden — we cannot order
    them, so UNRESOLVED. Never a guessed verdict.

    Same SSoT thresholds as classify_trade; entry_price is the official D1_Open
    basis (TASK-142). Analysis only — does NOT feed official metrics.
    """
    if entry_price is None or entry_price <= 0:
        return "UNRESOLVED"
    if minute_bars_df is None or minute_bars_df.empty:
        return "UNRESOLVED"
    if "low" not in minute_bars_df.columns or "high" not in minute_bars_df.columns:
        return "UNRESOLVED"
    ...
    tp_hits = df.index[df["low"] <= tp_price]
    sl_hits = df.index[df["high"] >= sl_price]
    if len(tp_hits) == 0 or len(sl_hits) == 0:
        return "UNRESOLVED"       # only one side (or neither) visible on minutes
    first_tp, first_sl = tp_hits[0], sl_hits[0]
    if first_tp < first_sl:
        return "WIN"
    if first_sl < first_tp:
        return "LOSS"
    return "UNRESOLVED"           # both first touched in the same minute
